...
4. **Drain-gate simplification**: mid-turn `session/send` now fails fast with
   `-32010` (whole-turn busy window; "1308" never existed in source) — the
   "accepted as steer and silently dropped" premise is obsolete. PENDING
   (core flow rewrite — needs its own change).
5. **`session/subagents`** → sub-agent status cards (waiting/blocked/lost
   detection); **`session/events` afterSeq** → gap-fill after drain-gate
   reloads instead of blind re-baseline. PENDING.
6. **`mcp/list mode:"status"`** → editor MCP health panel incl. OAuth
...
9. **`turn.completed` `resultType`/`cacheStats`** → rich terminal lines and
   cache-health visibility. PENDING.
